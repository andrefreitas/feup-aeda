...
customer::customer(string name, string birthDate, string address,
	...
	t= time(NULL);
	local=localtime(&t);
	this->lastOperation.day=local->tm_mday;
	this->lastOperation.mon=local->tm_mon+1;
	this->lastOperation.year=local->tm_year+1900;
	this->isInative=false;
}

// Overloaded constructor - for use with loadFile
customer::customer(string name, string birthDate, string address,
		string postalCode, nif NIF, long phone, string lastOperationp, bool isinactive,unsigned int ID) :
		person(name, birthDate, address, postalCode, NIF, phone) {
	this->id = ID;
	date auxDate(lastOperationp);
	this->isInative=isinactive;
	this->lastOperation=auxDate;
...
particular::particular(string name, string birthDate, string address,
		string postalCode, nif NIF, long BI, long phone, string lastOperationp, bool isinactive ,unsigned int ID) :
		customer(name, birthDate, address, postalCode, NIF, phone, lastOperationp,isinactive,ID) {
	this->BI = BI;
}
date customer::getLastOperation()
{
	return this->lastOperation;
}

void customer::setLastOperation(date new_date)
{
	this->lastOperation=new_date;
}
...
// overloaded constructor
company::company(string name, string birthDate, string address,
		string postalCode, nif NIF, long phone, double capital,  string lastOperationp, bool isinactive ,unsigned int ID) :
		customer(name, birthDate, address, postalCode, NIF, phone, lastOperationp, isinactive,ID) {
	this->capital = capital;
}
